...
data<-read.csv("C:/Matheus/Universidade/Mestrado/Planilhas/Dados NE/Population_parameters.csv",
...
{# F.subtilis N #
  ...
  #25% worst scenario
  lines(x=c(mean(F_fs_n)+0.25*mean(F_fs_n),mean(F_fs_n)+0.25*mean(F_fs_n)),
        y=c(min(fs_n_yr$Lc),mean(Lc_fs_n)-0.25*mean(Lc_fs_n)),lty=3,lwd=1,col='red')
  lines(x=c(min(fs_n_yr$FM_change),mean(F_fs_n)+0.25*mean(F_fs_n)),
        y=c(mean(Lc_fs_n)-0.25*mean(Lc_fs_n),mean(Lc_fs_n)-0.25*mean(Lc_fs_n)),col='red',lty=3,lwd=1)
  text(x=1*(mean(F_fs_n)+0.25*mean(F_fs_n)),y=0.35*min(fs_n_yr$Lc),
                                                              expression(F[wst]),cex=1,col = "red")
  text(x=min(fs_n_yr$FM_change)-0.21,y=mean(Lc_fs_n)-0.25*mean(Lc_fs_n),
                                                              expression(Lc["wst"]),cex=1,col = "red")
  #25% worst point
  points(mean(F_fs_n)+0.25*mean(F_fs_n), mean(Lc_fs_n)-0.25*mean(Lc_fs_n),col="red",pch=20)
  
  #25% best scenario
  lines(x=c(mean(F_fs_n)-0.25*mean(F_fs_n),mean(F_fs_n)-0.25*mean(F_fs_n)),
    y=c(min(fs_n_yr$Lc),mean(Lc_fs_n)+0.25*mean(Lc_fs_n)),lty=3,lwd=1,col='blue')
  lines(x=c(min(fs_n_yr$FM_change),mean(F_fs_n)-0.25*mean(F_fs_n)),
    y=c(mean(Lc_fs_n)+0.25*mean(Lc_fs_n),mean(Lc_fs_n)+0.25*mean(Lc_fs_n)),col='blue',lty=3,lwd=1)
  text(x=1*(mean(F_fs_n)-0.25*mean(F_fs_n)),y=0.35*min(fs_n_yr$Lc),
    expression(F[bst]),cex=1,col = "blue")
  text(x=min(fs_n_yr$FM_change)-0.21,y=mean(Lc_fs_n)+0.25*mean(Lc_fs_n),
    expression(Lc["bst"]),cex=1,col = "blue")
  #25% best point
  points(mean(F_fs_n)-0.25*mean(F_fs_n),mean(Lc_fs_n)+0.25*mean(Lc_fs_n),col="blue",pch=20)
  ...
  #25% worst scenario
  lines(x=c(mean(F_fs_ne)+0.25*mean(F_fs_ne),mean(F_fs_ne)+0.25*mean(F_fs_ne)),
    y=c(min(fs_ne_yr$Lc),mean(Lc_fs_ne)-0.25*mean(Lc_fs_ne)),lty=3,lwd=1,col='red')
  lines(x=c(min(fs_ne_yr$FM_change),mean(F_fs_ne)+0.25*mean(F_fs_ne)),
    y=c(mean(Lc_fs_ne)-0.25*mean(Lc_fs_ne),mean(Lc_fs_ne)-0.25*mean(Lc_fs_ne)),col='red',lty=3,lwd=1)
  text(x=1*(mean(F_fs_ne)+0.25*mean(F_fs_ne)),y=0.35*min(fs_ne_yr$Lc),
    expression(F[wst]),cex=1,col = "red")
  text(x=min(fs_ne_yr$FM_change)-0.25,y=mean(Lc_fs_ne)-0.25*mean(Lc_fs_ne),
    expression(Lc["wst"]),cex=1,col = "red")
  #25% worst point
  points(mean(F_fs_ne)+0.25*mean(F_fs_ne), mean(Lc_fs_ne)-0.25*mean(Lc_fs_ne),col="red",pch=20)
  
  #25% best scenario
  lines(x=c(mean(F_fs_ne)-0.25*mean(F_fs_ne),mean(F_fs_ne)-0.25*mean(F_fs_ne)),
    y=c(min(fs_ne_yr$Lc),mean(Lc_fs_ne)+0.25*mean(Lc_fs_ne)),lty=3,lwd=1,col='blue')
  lines(x=c(min(fs_ne_yr$FM_change),mean(F_fs_ne)-0.25*mean(F_fs_ne)),
    y=c(mean(Lc_fs_ne)+0.25*mean(Lc_fs_ne),mean(Lc_fs_ne)+0.25*mean(Lc_fs_ne)),col='blue',lty=3,lwd=1)
  text(x=1*(mean(F_fs_ne)-0.25*mean(F_fs_ne)),y=0.35*min(fs_ne_yr$Lc),
    expression(F[bst]),cex=1,col = "blue")
  text(x=min(fs_ne_yr$FM_change)-0.25,y=mean(Lc_fs_ne)+0.25*mean(Lc_fs_ne),
    expression(Lc["bst"]),cex=1,col = "blue")
  #25% best point
  points(mean(F_fs_ne)-0.25*mean(F_fs_ne),mean(Lc_fs_ne)+0.25*mean(Lc_fs_ne),col="blue",pch=20)
...
#25% worst scenario
lines(x=c(mean(F_fb_se)+0.25*mean(F_fb_se),mean(F_fb_se)+0.25*mean(F_fb_se)),
  y=c(min(fb_se_yr$Lc),mean(Lc_fb_se)-0.25*mean(Lc_fb_se)),lty=3,lwd=1,col='red')
lines(x=c(min(fb_se_yr$FM_change),mean(F_fb_se)+0.25*mean(F_fb_se)),
  y=c(mean(Lc_fb_se)-0.25*mean(Lc_fb_se),mean(Lc_fb_se)-0.25*mean(Lc_fb_se)),col='red',lty=3,lwd=1)
text(x=1*(mean(F_fb_se)+0.25*mean(F_fb_se)),y=0.64*min(fb_se_yr$Lc),
  expression(F[wst]),cex=1,col = "red")
text(x=min(fb_se_yr$FM_change)-0.47,y=mean(Lc_fb_se)-0.25*mean(Lc_fb_se),
  expression(Lc["wst"]),cex=1,col = "red")
#25% worst point
points(mean(F_fb_se)+0.25*mean(F_fb_se), mean(Lc_fb_se)-0.25*mean(Lc_fb_se),col="red",pch=20)

#25% best scenario
lines(x=c(mean(F_fb_se)-0.25*mean(F_fb_se),mean(F_fb_se)-0.25*mean(F_fb_se)),
  y=c(min(fb_se_yr$Lc),mean(Lc_fb_se)+0.25*mean(Lc_fb_se)),lty=3,lwd=1,col='blue')
lines(x=c(min(fb_se_yr$FM_change),mean(F_fb_se)-0.25*mean(F_fb_se)),
  y=c(mean(Lc_fb_se)+0.25*mean(Lc_fb_se),mean(Lc_fb_se)+0.25*mean(Lc_fb_se)),col='blue',lty=3,lwd=1)
text(x=1*(mean(F_fb_se)-0.25*mean(F_fb_se)),y=0.64*min(fb_se_yr$Lc),
  expression(F[bst]),cex=1,col = "blue")
text(x=min(fb_se_yr$FM_change)-0.47,y=mean(Lc_fb_se)+0.25*mean(Lc_fb_se),
  expression(Lc["bst"]),cex=1,col = "blue")
#25% best point
points(mean(F_fb_se)-0.25*mean(F_fb_se),mean(Lc_fb_se)+0.25*mean(Lc_fb_se),col="blue",pch=20)
#----------------------------------------------------------------------------------------
...
#25% worst scenario
lines(x=c(mean(F_fp_se)+0.25*mean(F_fp_se),mean(F_fp_se)+0.25*mean(F_fp_se)),
  y=c(min(fp_se_yr$Lc),mean(Lc_fp_se)-0.25*mean(Lc_fp_se)),lty=3,lwd=1,col='red')
lines(x=c(min(fp_se_yr$FM_change),mean(F_fp_se)+0.25*mean(F_fp_se)),
  y=c(mean(Lc_fp_se)-0.25*mean(Lc_fp_se),mean(Lc_fp_se)-0.25*mean(Lc_fp_se)),col='red',lty=3,lwd=1)
text(x=1*(mean(F_fp_se)+0.25*mean(F_fp_se)),y=0.84*min(fp_se_yr$Lc),
  expression(F[wst]),cex=1,col = "red")
text(x=min(fp_se_yr$FM_change)-0.57,y=mean(Lc_fp_se)-0.25*mean(Lc_fp_se),
  expression(Lc["wst"]),cex=1,col = "red")
#25% worst point
points(mean(F_fp_se)+0.25*mean(F_fp_se), mean(Lc_fp_se)-0.25*mean(Lc_fp_se),col="red",pch=20)

#25% best scenario
lines(x=c(mean(F_fp_se)-0.25*mean(F_fp_se),mean(F_fp_se)-0.25*mean(F_fp_se)),
  y=c(min(fp_se_yr$Lc),mean(Lc_fp_se)+0.25*mean(Lc_fp_se)),lty=3,lwd=1,col='blue')
lines(x=c(min(fp_se_yr$FM_change),mean(F_fp_se)-0.25*mean(F_fp_se)),
  y=c(mean(Lc_fp_se)+0.25*mean(Lc_fp_se),mean(Lc_fp_se)+0.25*mean(Lc_fp_se)),col='blue',lty=3,lwd=1)
text(x=1*(mean(F_fp_se)-0.25*mean(F_fp_se)),y=0.84*min(fp_se_yr$Lc),
  expression(F[bst]),cex=1,col = "blue")
text(x=min(fp_se_yr$FM_change)-0.57,y=mean(Lc_fp_se)+0.25*mean(Lc_fp_se),
  expression(Lc["bst"]),cex=1,col = "blue")
#25% best point
points(mean(F_fp_se)-0.25*mean(F_fp_se),mean(Lc_fp_se)+0.25*mean(Lc_fp_se),col="blue",pch=20)
...
#25% worst scenario
lines(x=c(mean(F_fp_s)+0.25*mean(F_fp_s),mean(F_fp_s)+0.25*mean(F_fp_s)),
  y=c(min(fp_s_yr$Lc),mean(Lc_fp_s)-0.25*mean(Lc_fp_s)),lty=3,lwd=1,col='red')
lines(x=c(min(fp_s_yr$FM_change),mean(F_fp_s)+0.25*mean(F_fp_s)),
  y=c(mean(Lc_fp_s)-0.25*mean(Lc_fp_s),mean(Lc_fp_s)-0.25*mean(Lc_fp_s)),col='red',lty=3,lwd=1)
text(x=1*(mean(F_fp_s)+0.25*mean(F_fp_s)),y=0.88*min(fp_s_yr$Lc),
  expression(F[wst]),cex=1,col = "red")
text(x=min(fp_s_yr$FM_change)-0.43,y=mean(Lc_fp_s)-0.25*mean(Lc_fp_s),
  expression(Lc["wst"]),cex=1,col = "red")
#25% worst point
points(mean(F_fp_s)+0.25*mean(F_fp_s), mean(Lc_fp_s)-0.25*mean(Lc_fp_s),col="red",pch=20)

#25% best scenario
lines(x=c(mean(F_fp_s)-0.25*mean(F_fp_s),mean(F_fp_s)-0.25*mean(F_fp_s)),
  y=c(min(fp_s_yr$Lc),mean(Lc_fp_s)+0.25*mean(Lc_fp_s)),lty=3,lwd=1,col='blue')
lines(x=c(min(fp_s_yr$FM_change),mean(F_fp_s)-0.25*mean(F_fp_s)),
  y=c(mean(Lc_fp_s)+0.25*mean(Lc_fp_s),mean(Lc_fp_s)+0.25*mean(Lc_fp_s)),col='blue',lty=3,lwd=1)
text(x=1*(mean(F_fp_s)-0.25*mean(F_fp_s)),y=0.88*min(fp_s_yr$Lc),
  expression(F[bst]),cex=1,col = "blue")
text(x=min(fp_s_yr$FM_change)-0.43,y=mean(Lc_fp_s)+0.25*mean(Lc_fp_s),
  expression(Lc["bst"]),cex=1,col = "blue")
#25% best point
points(mean(F_fp_s)-0.25*mean(F_fp_s),mean(Lc_fp_s)+0.25*mean(Lc_fp_s),col="blue",pch=20)
...
#average point
#points(mean(F_ls_n), mean(Lc_ls_n),col="black",pch=16)

#25% worst scenario
lines(x=c(min(ls_n_yr$FM_change),max(ls_n_yr$FM_change)),y=c(mean(Lc_ls_n)-0.25*mean(Lc_ls_n),mean(Lc_ls_n)-0.25*mean(Lc_ls_n)),lty=3,lwd=1,col='red')
text(x=min(ls_n_yr$FM_change)-0.26,y=mean(Lc_ls_n)-0.25*mean(Lc_ls_n),
  expression(Lc["wst"]),cex=1,col = "red")

#25% best scenario
lines(x=c(min(ls_n_yr$FM_change),max(ls_n_yr$FM_change)),y=c(mean(Lc_ls_n)+0.25*mean(Lc_ls_n),mean(Lc_ls_n)+0.25*mean(Lc_ls_n)),lty=3,lwd=1,col='blue')
text(x=min(ls_n_yr$FM_change)-0.26,y=mean(Lc_ls_n)+0.25*mean(Lc_ls_n),
  expression(Lc["bst"]),cex=1,col = "blue")
...
# litopenaeus schmitti NE #
  ...
  #25% worst scenario
  lines(x=c(mean(F_ls_ne)+0.25*mean(F_ls_ne),mean(F_ls_ne)+0.25*mean(F_ls_ne)),
    y=c(min(ls_ne_yr$Lc),mean(Lc_ls_ne)-0.25*mean(Lc_ls_ne)),lty=3,lwd=1,col='red')
  lines(x=c(min(ls_ne_yr$FM_change),mean(F_ls_ne)+0.25*mean(F_ls_ne)),
    y=c(mean(Lc_ls_ne)-0.25*mean(Lc_ls_ne),mean(Lc_ls_ne)-0.25*mean(Lc_ls_ne)),col='red',lty=3,lwd=1)
  text(x=1*(mean(F_ls_ne)+0.25*mean(F_ls_ne)),y=0.35*min(ls_ne_yr$Lc),
    expression(F[wst]),cex=1,col = "red")
  text(x=min(ls_ne_yr$FM_change)-0.21,y=mean(Lc_ls_ne)-0.25*mean(Lc_ls_ne),
    expression(Lc["wst"]),cex=1,col = "red")
  #25% worst point
  points(mean(F_ls_ne)+0.25*mean(F_ls_ne), mean(Lc_ls_ne)-0.25*mean(Lc_ls_ne),col="red",pch=20)
  
  #25% best scenario
  lines(x=c(mean(F_ls_ne)-0.25*mean(F_ls_ne),mean(F_ls_ne)-0.25*mean(F_ls_ne)),
    y=c(min(ls_ne_yr$Lc),mean(Lc_ls_ne)+0.25*mean(Lc_ls_ne)),lty=3,lwd=1,col='blue')
  lines(x=c(min(ls_ne_yr$FM_change),mean(F_ls_ne)-0.25*mean(F_ls_ne)),
    y=c(mean(Lc_ls_ne)+0.25*mean(Lc_ls_ne),mean(Lc_ls_ne)+0.25*mean(Lc_ls_ne)),col='blue',lty=3,lwd=1)
  text(x=1*(mean(F_ls_ne)-0.25*mean(F_ls_ne)),y=0.35*min(ls_ne_yr$Lc),
    expression(F[bst]),cex=1,col = "blue")
  text(x=min(ls_ne_yr$FM_change)-0.21,y=mean(Lc_ls_ne)+0.25*mean(Lc_ls_ne),
    expression(Lc["bst"]),cex=1,col = "blue")
  #25% best point
  points(mean(F_ls_ne)-0.25*mean(F_ls_ne),mean(Lc_ls_ne)+0.25*mean(Lc_ls_ne),col="blue",pch=20)
  ...
  #25% worst scenario
  lines(x=c(mean(F_ls_se)+0.25*mean(F_ls_se),mean(F_ls_se)+0.25*mean(F_ls_se)),
    y=c(min(ls_se_yr$Lc),mean(Lc_ls_se)-0.25*mean(Lc_ls_se)),lty=3,lwd=1,col='red')
  lines(x=c(min(ls_se_yr$FM_change),mean(F_ls_se)+0.25*mean(F_ls_se)),
    y=c(mean(Lc_ls_se)-0.25*mean(Lc_ls_se),mean(Lc_ls_se)-0.25*mean(Lc_ls_se)),col='red',lty=3,lwd=1)
  text(x=1*(mean(F_ls_se)+0.25*mean(F_ls_se)),y=0.85*min(ls_se_yr$Lc),
    expression(F[wst]),cex=1,col = "red")
  text(x=min(ls_se_yr$FM_change)-0.32,y=mean(Lc_ls_se)-0.25*mean(Lc_ls_se),
    expression(Lc["wst"]),cex=1,col = "red")
  #25% worst point
  points(mean(F_ls_se)+0.25*mean(F_ls_se), mean(Lc_ls_se)-0.25*mean(Lc_ls_se),col="red",pch=20)
  
  #25% best scenario
  lines(x=c(mean(F_ls_se)-0.25*mean(F_ls_se),mean(F_ls_se)-0.25*mean(F_ls_se)),
    y=c(min(ls_se_yr$Lc),mean(Lc_ls_se)+0.25*mean(Lc_ls_se)),lty=3,lwd=1,col='blue')
  lines(x=c(min(ls_se_yr$FM_change),mean(F_ls_se)-0.25*mean(F_ls_se)),
    y=c(mean(Lc_ls_se)+0.25*mean(Lc_ls_se),mean(Lc_ls_se)+0.25*mean(Lc_ls_se)),col='blue',lty=3,lwd=1)
  text(x=1*(mean(F_ls_se)-0.25*mean(F_ls_se)),y=0.85*min(ls_se_yr$Lc),
    expression(F[bst]),cex=1,col = "blue")
  text(x=min(ls_se_yr$FM_change)-0.32,y=mean(Lc_ls_se)+0.25*mean(Lc_ls_se),
    expression(Lc["bst"]),cex=1,col = "blue")
  #25% best point
  points(mean(F_ls_se)-0.25*mean(F_ls_se),mean(Lc_ls_se)+0.25*mean(Lc_ls_se),col="blue",pch=20)
...
#25% worst scenario
lines(x=c(min(xk_n_yr$FM_change),max(xk_n_yr$FM_change)),y=c(mean(Lc_xk_n)-0.25*mean(Lc_xk_n),mean(Lc_xk_n)-0.25*mean(Lc_xk_n)),lty=3,lwd=1,col='red')
text(x=min(xk_n_yr$FM_change)-0.32,y=mean(Lc_xk_n)-0.25*mean(Lc_xk_n),
  expression(Lc["wst"]),cex=1,col = "red")

#25% best scenario
lines(x=c(min(xk_n_yr$FM_change),max(xk_n_yr$FM_change)),y=c(mean(Lc_xk_n)+0.25*mean(Lc_xk_n),mean(Lc_xk_n)+0.25*mean(Lc_xk_n)),lty=3,lwd=1,col='blue')
text(x=min(xk_n_yr$FM_change)-0.32,y=mean(Lc_xk_n)+0.25*mean(Lc_xk_n),
  expression(Lc["bst"]),cex=1,col = "blue")
#---------------------------------------------------------------------------------------------
...
#25% worst scenario
lines(x=c(mean(F_xk_ne)+0.25*mean(F_xk_ne),mean(F_xk_ne)+0.25*mean(F_xk_ne)),
  y=c(min(xk_ne_yr$Lc),mean(Lc_xk_ne)-0.25*mean(Lc_xk_ne)),lty=3,lwd=1,col='red')
lines(x=c(min(xk_ne_yr$FM_change),mean(F_xk_ne)+0.25*mean(F_xk_ne)),
  y=c(mean(Lc_xk_ne)-0.25*mean(Lc_xk_ne),mean(Lc_xk_ne)-0.25*mean(Lc_xk_ne)),col='red',lty=3,lwd=1)
text(x=1*(mean(F_xk_ne)+0.25*mean(F_xk_ne)),y=0.77*min(xk_ne_yr$Lc),
  expression(F[wst]),cex=1,col = "red")
text(x=min(xk_ne_yr$FM_change)-0.4,y=mean(Lc_xk_ne)-0.25*mean(Lc_xk_ne),
  expression(Lc["wst"]),cex=1,col = "red")
#25% worst point
points(mean(F_xk_ne)+0.25*mean(F_xk_ne), mean(Lc_xk_ne)-0.25*mean(Lc_xk_ne),col="red",pch=20)

#25% best scenario
lines(x=c(mean(F_xk_ne)-0.25*mean(F_xk_ne),mean(F_xk_ne)-0.25*mean(F_xk_ne)),
  y=c(min(xk_ne_yr$Lc),mean(Lc_xk_ne)+0.25*mean(Lc_xk_ne)),lty=3,lwd=1,col='blue')
lines(x=c(min(xk_ne_yr$FM_change),mean(F_xk_ne)-0.25*mean(F_xk_ne)),
  y=c(mean(Lc_xk_ne)+0.25*mean(Lc_xk_ne),mean(Lc_xk_ne)+0.25*mean(Lc_xk_ne)),col='blue',lty=3,lwd=1)
text(x=1*(mean(F_xk_ne)-0.25*mean(F_xk_ne)),y=0.77*min(xk_ne_yr$Lc),
  expression(F[bst]),cex=1,col = "blue")
text(x=min(xk_ne_yr$FM_change)-0.4,y=mean(Lc_xk_ne)+0.25*mean(Lc_xk_ne),
  expression(Lc["bst"]),cex=1,col = "blue")
#25% best point
points(mean(F_xk_ne)-0.25*mean(F_xk_ne),mean(Lc_xk_ne)+0.25*mean(Lc_xk_ne),col="blue",pch=20)
...
#25% worst scenario
lines(x=c(mean(F_xk_se)+0.25*mean(F_xk_se),mean(F_xk_se)+0.25*mean(F_xk_se)),
  y=c(min(xk_se_yr$Lc),mean(Lc_xk_se)-0.25*mean(Lc_xk_se)),lty=3,lwd=1,col='red')
lines(x=c(min(xk_se_yr$FM_change),mean(F_xk_se)+0.25*mean(F_xk_se)),
  y=c(mean(Lc_xk_se)-0.25*mean(Lc_xk_se),mean(Lc_xk_se)-0.25*mean(Lc_xk_se)),col='red',lty=3,lwd=1)
text(x=1*(mean(F_xk_se)+0.25*mean(F_xk_se)),y=0.9*min(xk_se_yr$Lc),
  expression(F[wst]),cex=1,col = "red")
text(x=min(xk_se_yr$FM_change)-0.3,y=mean(Lc_xk_se)-0.25*mean(Lc_xk_se),
  expression(Lc["wst"]),cex=1,col = "red")
#25% worst point
points(mean(F_xk_se)+0.25*mean(F_xk_se), mean(Lc_xk_se)-0.25*mean(Lc_xk_se),col="red",pch=20)

#25% best scenario
lines(x=c(mean(F_xk_se)-0.25*mean(F_xk_se),mean(F_xk_se)-0.25*mean(F_xk_se)),
  y=c(min(xk_se_yr$Lc),mean(Lc_xk_se)+0.25*mean(Lc_xk_se)),lty=3,lwd=1,col='blue')
lines(x=c(min(xk_se_yr$FM_change),mean(F_xk_se)-0.25*mean(F_xk_se)),
  y=c(mean(Lc_xk_se)+0.25*mean(Lc_xk_se),mean(Lc_xk_se)+0.25*mean(Lc_xk_se)),col='blue',lty=3,lwd=1)
text(x=1*(mean(F_xk_se)-0.25*mean(F_xk_se)),y=0.9*min(xk_se_yr$Lc),
  expression(F[bst]),cex=1,col = "blue")
text(x=min(xk_se_yr$FM_change)-0.3,y=mean(Lc_xk_se)+0.25*mean(Lc_xk_se),
  expression(Lc["bst"]),cex=1,col = "blue")
#25% best point
points(mean(F_xk_se)-0.25*mean(F_xk_se),mean(Lc_xk_se)+0.25*mean(Lc_xk_se),col="blue",pch=20)
